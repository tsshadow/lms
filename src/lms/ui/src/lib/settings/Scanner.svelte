<script>
  ...
  function getScanDuration() {
    if (!status?.lastScan?.startTime || !status?.lastScan?.stopTime) return 'N/A';
    return Math.round((new Date(status.lastScan.stopTime).getTime() - new Date(status.lastScan.startTime).getTime()) / 1000) + 's';
  }
