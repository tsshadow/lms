    ...
    {#each tracks as track, i}
      ...
      >
        <td class="p-2 px-4 text-right relative">
            ...
            <button class="hidden group-hover:block bg-transparent border-none text-white cursor-pointer p-0 absolute right-4 top-1/2 -translate-y-1/2" on:click={() => playTrack(track)}>
        ...
        {#if isQueue}
          <td class="p-2 px-4 text-center">
            <div class="flex gap-1 justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                <button class="bg-transparent border-none text-[#b3b3b3] cursor-pointer p-1 flex items-center justify-center rounded hover:bg-[#333] hover:text-white disabled:text-[#555] disabled:cursor-not-allowed" on:click={(e) => moveTrack(e, i, -1)} disabled={i === 0}>
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
                        <path d="M7 14l5-5 5 5z"></path>
                    </svg>
                </button>
                <button class="bg-transparent border-none text-[#b3b3b3] cursor-pointer p-1 flex items-center justify-center rounded hover:bg-[#333] hover:text-white disabled:text-[#555] disabled:cursor-not-allowed" on:click={(e) => moveTrack(e, i, 1)} disabled={i === tracks.length - 1}>
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
                        <path d="M7 10l5 5 5-5z"></path>
                    </svg>
                </button>
                <button class="bg-transparent border-none text-[#b3b3b3] cursor-pointer p-1 flex items-center justify-center rounded hover:bg-[#333] hover:text-white hover:text-pink-500" on:click={(e) => removeTrack(e, i)}>
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
