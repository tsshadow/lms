...
CREATE TABLE IF NOT EXISTS "track_backup" (
  ...
  constraint "fk_track_release" foreign key ("release_id") references "release" ("id") on delete cascade deferrable initially deferred
            ...
            utils::executeCommand(*session.getDboSession(), "INSERT INTO track_backup SELECT id, version, scan_version, track_number, rating, disc_number, total_track, disc_subtitle, name, duration, date, original_date, file_path, file_last_write, file_added, has_cover, mbid, recording_mbid, copyright, copyright_url, track_replay_gain, release_replay_gain, release_id FROM track");
...
FROM track)");
  ...
  constraint "fk_release_artist_link_artist" foreign key ("artist_id") references "artist" ("id") on delete cascade deferrable initially deferred
    ...
    {
        utils::executeCommand(*session.getDboSession(), "ALTER TABLE track ADD COLUMN play_count INTEGER DEFAULT 0");
        utils::executeCommand(*session.getDboSession(), "ALTER TABLE track ADD COLUMN last_played TEXT");
    }
