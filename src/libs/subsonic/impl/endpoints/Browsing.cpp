    ...
    namespace
    {
        ...
        std::vector<TrackId> findSimilarSongs(RequestContext& context, ReleaseId releaseId, std::size_t count)
        {
            ...
            std::vector<ReleaseId> releaseIds{
                core::Service<recommendation::IRecommendationService>::get()->getSimilarReleases(
                    releaseId, similarReleaseCount)
            };
        ...
        Response handleGetSimilarSongsRequestCommon(RequestContext& context, bool id3)
        {
            ...
            Response::Node& similarSongsNode{
                response.createNode(id3 ? Response::Node::Key{ "similarSongs2" } : Response::Node::Key{ "similarSongs" })
            };
