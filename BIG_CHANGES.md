...
*   `getSpotifyCuratedPlaylists`: Direct access to "Dynamic Playlists" managed via the MuMa Control Center.
*   `spotifyImportFromListenBrainz`: Synchronizes listening history from external scrobbling services.

### 3. Dynamic (Smart) Playlists
Rule-based playlists that update in real-time. Instead of static lists of songs, these playlists use JSON-defined parameters (genre, rating, release year, etc.) to dynamically populate tracks. They are fully managed through the MuMa Control Center.

### 4. Festival Lineups & Cluster Filtering
...
*   Integrated smart playlists and curated festival sections.
*   Synchronized ratings and play counts across the MuMa ecosystem.

### 6. Modern Spotify-like UI (Svelte)
